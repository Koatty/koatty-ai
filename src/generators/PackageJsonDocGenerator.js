...
exports.ensureDocScriptInPackageJson = ensureDocScriptInPackageJson;
 ...
 */
function ensureDocScriptInPackageJson(changeset, workingDirectory, options = { addScriptFile: true }) {
    ...
    if (modified) {
        ...
        changeset.modifyFile('package.json', newContent, originalContent, 'Add doc script for Typia API documentation');
    }
    if (options.addScriptFile) {
        const scriptPath = path.join(workingDirectory, 'scripts', 'generate-api-doc.ts');
        if (!fs.existsSync(scriptPath)) {
            const templatePath = path.join(__dirname, '..', '..', 'templates', 'scripts', 'generate-api-doc.ts');
            const content = fs.existsSync(templatePath)
                ? fs.readFileSync(templatePath, 'utf-8')
                : getDefaultDocScriptContent();
            changeset.createFile('scripts/generate-api-doc.ts', content, 'Add API doc generator script (Typia + controllers)');
        }
    }
}
function getDefaultDocScriptContent() {
    return `/**
...
//# sourceMappingURL=PackageJsonDocGenerator.js.map