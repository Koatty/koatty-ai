...
function registerGenerateCommand(program) {
    const generate = program
        ...
        const spinner = (0, ora_1.default)(`Generating module: ${moduleName}`).start();
        try {
            ...
            const spec = GeneratorPipeline_1.GeneratorPipeline.buildSpecFromOptions(moduleName, pipelineOptions);
            // Step 2: Merge inline fields if provided
            if (options.fields) {
                const tempPipeline = new GeneratorPipeline_1.GeneratorPipeline(spec);
            ...
            if (options.config) {
                ...
                const configPipeline = new GeneratorPipeline_1.GeneratorPipeline(configPath);
