...
export function registerCreateCommands(program: Command): void {
  program
    ...
    .action((name: string | undefined, opts: { type?: string }) => {
      const n = (name || '').trim();
      if (!n) {
        console.error('请提供名称，例如: koatty controller user 或 koatty controller user -t grpc');
        process.exit(1);
      }
      runCreateModule('controller', n, { type: opts.type });
    });
  ...
  program
    ...
    .action((name: string) => {
      runCreateModule('middleware', name.trim());
    });
  ...
  program
    ...
    .action((name: string, opts: { interface?: boolean }) => {
      runCreateModule('service', name.trim(), { interface: opts.interface });
    });
  ...
  program
    ...
    .action((name: string) => {
      runCreateModule('plugin', name.trim());
    });
  ...
  program
    ...
    .action((name: string) => {
      runCreateModule('aspect', name.trim());
    });
  ...
  program
    ...
    .action((name: string) => {
      runCreateModule('exception', name.trim());
    });
  ...
  program
    ...
    .action((name: string) => {
      runCreateModule('proto', name.trim());
    });
  ...
  program
    ...
    .action((name: string, opts: { orm?: string }) => {
      runCreateModule('model', name.trim(), { orm: opts.orm });
    });
}
