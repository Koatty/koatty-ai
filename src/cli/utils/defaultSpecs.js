...
exports.getDefaultFieldsForModule = getDefaultFieldsForModule;
exports.parseFieldShortSpec = parseFieldShortSpec;
exports.hasDefaultForModule = hasDefaultForModule;
const VALID_TYPES = ['string', 'number', 'boolean', 'enum', 'datetime', 'text', 'json'];
/**
 ...
 */
function getDefaultFieldsForModule(moduleName) {
 ...
 */
function parseFieldShortSpec(input) {
    const fields = {};
    const parts = input
        .trim()
        .split(/\s+/)
        .filter(Boolean);
    const modifiers = new Set(['required', 'unique', 'nullable']);
            ...
            if (modifiers.has(part.toLowerCase())) {
                ...
                if (lastKey && fields[lastKey]) {
                    if (part.toLowerCase() === 'required')
                        fields[lastKey].required = true;
                    if (part.toLowerCase() === 'unique')
                        fields[lastKey].unique = true;
                    if (part.toLowerCase() === 'nullable')
                        fields[lastKey].nullable = true;
                }
            }
            continue;
        }
...
function hasDefaultForModule(moduleName) {
    return !!MODULE_DEFAULTS[moduleName.toLowerCase()];
}
//# sourceMappingURL=defaultSpecs.js.map