...
class TemplateManager {
     ...
     */
    async downloadTemplate(type, mirror, onProgress) {
     ...
     * @returns 模板目录路径
     */
    async ensureTemplateRepo(type, options) {
        if (options?.force) {
     ...
     * @param onProgress 进度回调（可选）
     */
    async updateTemplates(mirror, onProgress) {
        const types = ['project', 'modules', 'component'];
     ...
     * @returns 渲染结果数组
     */
    async renderDirectory(templateDir, context, options) {
        const results = [];
     ...
     * @returns 替换变量后的路径
     */
    renderPathVariables(filePath, context) {
...
/** 模板仓库地址配置 */
TemplateManager.TEMPLATE_REPOS = {
    project: {
...
TemplateManager.helpersRegistered = false;
